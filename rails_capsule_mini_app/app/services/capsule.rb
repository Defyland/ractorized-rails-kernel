...
# The capsule protocol, as REAL Rails app code (autoloaded/eager-loaded by Zeitwerk).
# Same shape and same semantics as the standalone-AR slice, now living inside a booted
# Rails environment and backed by Postgres primitives:
#   Proposal (versioned) -> CommitCoordinator (one AR transaction: optimistic update + outbox
#   insert) -> OutboxDispatcher (FOR UPDATE SKIP LOCKED, post-commit) -> idempotent ExternalConsumer.
#
...
module Capsule
  ...
  # It reads the Rails global (Current.user) ONCE, on the main Ractor, and snapshots primitives.
  CapsuleContext = Data.define(:user_id, :user_name) do
    def self.from_current
      if Current.user.nil?
        raise CapsuleContextRequired, "Current.user is nil; a capsule needs an explicit CapsuleContext"
      end
  ...
  # A shareable snapshot of the mutable Order, built by the OWNER (reads ActiveRecord ONCE). Passing
  # this — instead of the live AR record — is what lets the proposal computation run INSIDE a Ractor:
  # the capsule never touches ActiveRecord, only frozen primitives.
