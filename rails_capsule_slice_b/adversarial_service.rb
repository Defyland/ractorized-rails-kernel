...
# Deliberately resistant Rails service code, plus its capsule transform. Each resistant pattern
# the synthetic Slice A classified is reproduced here against the REAL stack and shown to be either
# locally transformable into a Ractor-shareable / side-effect-free form, or to fail by an EXPLICIT
# NAMED error (never silently). Real Ractor exception classes (Ruby 4.0.5, verified):
#   make_shareable(capturing proc)          -> Ractor::IsolationError
#   Ractor.new { ...outer non-shareable...} -> ArgumentError
...
class AdversarialService
  ...
  # Touches the ambient Current global, captures a live AR object in a proc, calls a native-ext
  # inline, and (worst) performs the external charge BEFORE — and outside — the DB commit.
  # ---------------------------------------------------------------------------
  ...
  # effect. Emits a Proposal; the external effect becomes a deferred EffectIntent (post-commit).
  # ---------------------------------------------------------------------------
  def capsule_propose(context:, order:, rate_bps:, request_id:)
    ...
    # ENFORCED boundary: refuse a live ActiveRecord record — only a shareable Capsule::OrderSnapshot
    # may cross into the capsule (this is what makes "runs in a Ractor" a real boundary).
  ...
  # a genuine Ractor::UnsafeError ("ractor unsafe method called from not main ractor"), which surfaces
  # through Ractor#value as Ractor::RemoteError whose #cause is the UnsafeError. The capsule classifies
  # it owner-only and runs the OWNER fallback (main Ractor). The routing is exercised against the real
  # C-ext, not a proxy.
  ...
  # We EXPECT that raise (it triggers the owner fallback), so silence Ruby's report_on_exception around
  # this intentional probe to keep the suite output clean — the exception is still raised, surfaced
